...
function Home() {
    ...
    const [toggleBtn, setToggleBtn] = useState('<i class="far fa-sun"></i> Light')
    ...
    const toggleDarkMode = () => {
        if(mode){
            ...
            setToggleBtn('<i class="fas fa-moon"></i> Dark')
            setMode(current => current = !current)
        }
        if(!mode) {
            document.documentElement.classList.remove('dark')
            setToggleBtn('<i class="far fa-sun"></i> Light')
            setMode(current => current = !current)
        }
    }
    ...
    return (    
        <div className="bg-gray-100 dark:bg-gray-800 dark:text-white">
            ...
            <div className="flex container mx-auto mb-16">
                <i className="fa fa-search my-auto -mr-9 z-10 pr-2 pl-3 py-5 rounded-md text-gray-400"/>
